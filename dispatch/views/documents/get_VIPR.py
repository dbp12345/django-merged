...
class DispatchDocsVIPRView(View):
    doc_path = Path(settings.BASE_DIR) / "core" / "documents_templates" / "manifests" / "VIPR Manifest Form FILLABLE.docx"
    output_path = Path(settings.BASE_DIR) / "core" / "documents_templates" / "manifests" / "VIPR Crew Manifest.docx"
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        employees_entries = dispatch_instance.crew.employees.all() if dispatch_instance.crew else []
        ...
        res = self.generate_docx_response(context, employees, vehicles, filename=f"VIPR Crew Manifest {crew_name}")

        return res

    def generate_docx_response(self, context, employees, vehicles, filename="VIPR Crew Manifest") -> HttpResponse:
